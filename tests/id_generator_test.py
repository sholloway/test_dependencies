...
class TestIdGenerator:
  ...
  def test_id_length(self) -> None:
    gen = IDGenerator()
    assert len(gen.id()) == 18

  def test_id_format(self) -> None:
    low_id_generator = IDGenerator()
    assert low_id_generator.id() == '01p0M0000000001QAY'
    assert low_id_generator.id() == '01p0M0000000002QAY'
    assert low_id_generator.id() == '01p0M0000000003QAY'
    assert low_id_generator.id() == '01p0M0000000004QAY'
    assert low_id_generator.id() == '01p0M0000000005QAY'

    high_id_generator = IDGenerator(starting_id=12_345_678_910)
    assert high_id_generator.id() == '01p0M0000dtvd3pQAY'
    assert high_id_generator.id() == '01p0M0000dtvd3qQAY'
    assert high_id_generator.id() == '01p0M0000dtvd3rQAY'